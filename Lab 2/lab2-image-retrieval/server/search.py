...
def get_top_k_similar(image_data, pred, pred_final, k):
    ...
    top_k_ind = np.argsort(
        [cosine(image_data, pred_row) for ith_row, pred_row in enumerate(pred)]
    )[:k]
    print(top_k_ind)
...
def recommend(imagePath, extracted_features):
    ...
    print("loaded images")
    return get_top_k_similar(features, extracted_features, neighbor_list, k=18)
